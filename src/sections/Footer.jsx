...
export default function Footer() {
  const socials = [
    ...
    },
    {
      ...
      color: "hover:text-pink-700 hover:border-pink-500/50",
    },
    {
      label: "TikTok",
      icon: "icon-[mdi--music-note]",
      href: "https://tiktok.com/@HcaTechdz",
      color: "hover:text-black hover:border-black/30",
    },
    {
      label: "YouTube",
  ...
  const links = [
    { label: "Catalogue", to: "/produits", isRoute: true },
    { label: "Avis clients", to: { pathname: "/", hash: "#testimonials" }, isRoute: true },
    { label: "Contact", to: { pathname: "/", hash: "#contact" }, isRoute: true },
  ...
  return (
    ...
    >
      <div className="px-4 sm:px-10 lg:px-20 py-10 sm:py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
        {/* Colonne 1 — Brand */}
        <div className="flex flex-col gap-4">
          ...
          <p className="text-sm leading-relaxed text-text-muted max-w-xs">
            Votre référence tech en Algérie. PC portables, moniteurs,
            accessoires gaming et équipements professionnels.
          </p>
            ...
            {links.map((l) => (
              <li key={l.label}>
                {l.isRoute ? (
                  <Link
                    to={l.to}
                    className="group inline-flex items-center gap-2 text-sm text-text-muted hover:text-text transition-colors duration-200"
                  >
                    <span
                      aria-hidden="true"
                      className="icon-[mdi--chevron-right] w-4 h-4 text-blue-500/50 group-hover:text-blue-500 transition-colors"
                    />
                    {l.label}
                  </Link>
                ) : (
          ...
          <ul className="flex flex-col gap-3 list-none p-0">
            {infos.map((info) => (
              <li key={info.value} className="flex items-center gap-3">
                <span
                  aria-hidden="true"
                  className={`${info.icon} w-4 h-4 text-primary shrink-0`}
                />
                {info.href ? (
                  <a
                    href={info.href}
      ...
      <div className="border-t border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-10 lg:px-20 py-4 sm:py-5 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-text-muted">
          <p>
            © 2026 <span className="text-primary font-semibold">HCA Tech</span>.
