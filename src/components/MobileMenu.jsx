...
const NAV_LINKS = [
  { label: "Accueil", to: "/", icon: "icon-[mdi--home-outline]" },
  { label: "Produits", to: "/produits", icon: "icon-[mdi--tag-outline]" },
  {
    label: "Avis Clients",
    ...
    <>
      ...
      <aside
        ...
        className={`
          ...
          flex flex-col
          bg-bg/95 border-l border-purple-500/30
          backdrop-blur-xl
      ...
      >
        ...
        >
          <p className="text-sm font-bold uppercase tracking-widest text-primary mb-3 px-1">
            Navigation
          </p>
            ...
            {NAV_LINKS.map(({ label, to, icon }) => (
              <li key={label}>
                <Link
                  to={to}
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-text text-[15px] font-semibold hover:bg-primary/10 hover:text-primary transition-all duration-200"
                >
                  <span
                    aria-hidden="true"
                    className={`${icon} w-4 h-4 shrink-0`}
                  />
                  {label}
                </Link>
              </li>
        ...
        <nav aria-label="Catégories produits" className="px-4 pb-6">
          <p className="text-sm font-bold uppercase tracking-widest text-purple-500 mb-3 px-1">
            Catégories
          </p>
          <ul className="space-y-1 list-none p-0">
            {categoriesData.map(({ id, title, icon }) => (
              <li key={id}>
                <Link
                  ...
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-text text-[15px] font-semibold hover:bg-purple-500/10 hover:text-purple-400 transition-all duration-200"
                >
                  <span
                    aria-hidden="true"
                  ...
                  <span
                    aria-hidden="true"
                    className="icon-[mdi--chevron-right] w-5 h-5 ml-auto text-purple-500"
                  />
                </Link>
              </li>
            ))}
